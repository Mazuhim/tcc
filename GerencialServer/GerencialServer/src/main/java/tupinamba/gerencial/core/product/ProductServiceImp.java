/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package tupinamba.gerencial.core.product;

import e_persistence.services.exceptions.ServiceException;
import e_persistence.session.Session;
import tupinamba.gerencial.persistence.GerencialServerService;

/**
 *
 * @author Mazuhim
 */
public class ProductServiceImp extends GerencialServerService<Product, Integer> implements ProductService {

    public ProductServiceImp(Session session) {
        super(Product.class, session);
    }

    @Override
    public void save(Product entity) throws ServiceException {
        this.begin();

        try {
            Session session = this.getSession();

            if (entity.getIdProduct() == 0) {
                session.create(entity);
            } else {
                session.edit(entity);
            }

            session.flush();
        } catch (Exception e) {
            this.rollback(e);
        }

        this.commit();
    }

}
